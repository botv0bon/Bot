      ...
      [
        {
          text: "🌳 Withdraw SOL",
          callback_data: JSON.stringify({
            command: `TF_${NATIVE_MINT.toString()}`,
          }),
        },
        {
          text: "↩️ Back",
...
export const withdrawAddressHandler = async (
  ...
  const balance =
    mint === NATIVE_MINT.toString()
      ...
      : await TokenService.getSPLBalance(
          mint,
          user.wallet_address,
          isToken2022
        );
...
export const withdrawHandler = async (
  ...
  const getcaption = async (status: string, suffix: string = "") => {
    const securecaption =
      `🌳 Token: <b>${tokenName ?? "undefined"} (${
        symbol ?? "undefined"
      })</b> ` +
      `${isToken2022 ? "<i>Token2022</i>" : ""}\n` +
      `<i>${copytoclipboard(mint)}</i>\n` +
      status +
      `💲 <b>Value: ${amount} ${symbol} ($ ${(amount * price).toFixed(
        3
      )})</b>\n` +
      suffix;
  ...
  const transferResult =
    mint === NATIVE_MINT.toString()
      ? await jupiterSerivce.transferSOL(
          amount,
          9,
          topubkey,
          user.private_key,
      ...
      : await jupiterSerivce.transferSPL(
          mint,
          amount,
          decimals,
          topubkey,
  ...
  if (transferResult) {
    ...
    const successCaption = await getcaption(
...
export const cancelWithdrawHandler = async (
  bot: TelegramBot,
  msg: TelegramBot.Message
) => {
  const chat_id = msg.chat.id;
