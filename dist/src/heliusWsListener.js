...
exports.getProtocolRules = getProtocolRules;
exports.getHeliusListenerStats = getHeliusListenerStats;
exports.resetHeliusListenerStats = resetHeliusListenerStats;
exports.startHeliusWebsocketListener = startHeliusWebsocketListener;
exports.getRecentHeliusEvents = getRecentHeliusEvents;
exports.saveLastInstance = saveLastInstance;
...
let _getBlockTimeForSlotCached = null;
let _getCachedBlockTimeForSlot = null;
try {
    ...
    _getBlockTimeForSlotCached = null;
    _getCachedBlockTimeForSlot = null;
}
...
let _slotBlocktimeWarmerHandle = null;
function startSlotBlocktimeWarmer(opts) {
    try {
        if (!(_getBlockTimeForSlotCached || _getCachedBlockTimeForSlot))
            return;
        if (_slotBlocktimeWarmerHandle)
            return;
        ...
        _slotBlocktimeWarmerHandle = setInterval(async () => {
            try {
                ...
                let tipSlot = null;
                try {
                    if (ff && typeof ff.getRecentSlot === 'function') {
                        tipSlot = await ff.getRecentSlot().catch(() => null);
                    }
                }
                catch (e) { }
                ...
                for (let s = to; s >= from; s--) {
                    ...
                    try {
                        ...
                        if (cached || cached === 0) {
                            ...
                            try {
                                await _getBlockTimeForSlotCached(s).catch(() => null);
                            }
                            catch (e) { }
                        }
                    }
...
function stopSlotBlocktimeWarmer() { try {
    if (_slotBlocktimeWarmerHandle) {
        clearInterval(_slotBlocktimeWarmerHandle);
        _slotBlocktimeWarmerHandle = null;
    }
}
catch (e) { } }
const HELIUS_WS_URL = (0, config_1.getHeliusWebsocketUrl)();
const USE_WS = config_1.HELIUS_USE_WEBSOCKET;
const SUBSCRIBE_METADATA = config_1.HELIUS_SUBSCRIBE_METADATA;
const SUBSCRIBE_SPLTOKEN = config_1.HELIUS_SUBSCRIBE_SPLTOKEN;
...
function loadProtocolRules() {
    try {
        if (!fs.existsSync(PROTOCOL_RULES_PATH))
            return null;
        ...
        for (const p of (parsed.protocols || [])) {
            if (p && p.pubkey)
                map[String(p.pubkey).toLowerCase()] = p;
        }
        parsed._map = map;
...
catch (e) { } }, Number(process.env.PROTOCOL_RULES_RELOAD_MS || 7000));
function getProtocolRules() { return PROTOCOL_RULES; }
...
const _quickCheckCache = new Map();
...
async function retryOfficialEnrich(tuMod, tokenObj, opts = {}) {
    try {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                if (!tuMod || typeof tuMod.officialEnrich !== 'function')
                    return null;
                await Promise.race([
...
async function quickIsTraded(mintAddr) {
    try {
        const now = Math.floor(Date.now() / 1000);
        const key = String(mintAddr).toLowerCase();
        const cached = _quickCheckCache.get(key);
        if (cached && (now - (cached.ts || 0) <= QUICK_CHECK_TTL_S) && typeof cached.isTraded === 'boolean')
            return cached.isTraded;
    }
    catch (e) { }
    try {
        const axios = require('axios');
        ...
        try {
            ...
            if (dRes && dRes.data) {
                ...
                if (Array.isArray(dRes.data.pairs) && dRes.data.pairs.length > 0)
                    return true;
                // some endpoints return an object where keys are pairs
                if (typeof dRes.data === 'object' && Object.keys(dRes.data).length > 0 && (dRes.data.pairs || dRes.data.pair))
                    return true;
            }
        }
        catch (e) { }
            ...
            if (resp && resp.data && Array.isArray(resp.data) && resp.data.length) {
                for (const tx of resp.data) {
                    try {
                        ...
                        if (bt && (nowS - Number(bt) <= HELIUS_FRESHNESS_THRESHOLD_S))
                            return true;
                        ...
                        for (const tt of tokenTransfers) {
                            if (tt && (tt.mint === mintAddr || tt.mint === String(mintAddr))) {
                                ...
                                if (Number(tt.amount || tt.tokenAmount || tt.uiAmount || 0) > 0)
                                    return true;
                            }
                        }
        ...
        try {
            _quickCheckCache.set(String(mintAddr).toLowerCase(), { isTraded: false, ts: Math.floor(Date.now() / 1000) });
        }
        catch (e) { }
        return false;
    }
...
async function quickIsTradedByDex(mintAddr) {
    try {
        const now = Math.floor(Date.now() / 1000);
        const key = String(mintAddr).toLowerCase();
        const cached = _quickCheckCache.get(key);
...
async function quickIsMintYoung(mintAddr, maxAgeSec = HELIUS_MAX_MINT_AGE_S) {
    try {
        const now = Math.floor(Date.now() / 1000);
        const key = String(mintAddr).toLowerCase();
        const cached = _quickCheckCache.get(key);
        if (cached && (now - (cached.ts || 0) <= QUICK_CHECK_TTL_S) && typeof cached.isYoung === 'boolean')
            return cached.isYoung;
    }
    catch (e) { }
    try {
        if (!mintAddr)
        ...
        try {
            if (ff && typeof ff.getMintSnapshotCached === 'function') {
                ...
                if (snap) {
                    ...
                    try {
                        _quickCheckCache.set(String(mintAddr).toLowerCase(), { isYoung: ok, ts: Math.floor(Date.now() / 1000) });
                    }
                    catch (e) { }
                    if (ok)
                        return true;
        ...
        try {
            _quickCheckCache.set(String(mintAddr).toLowerCase(), { isYoung: false, ts: Math.floor(Date.now() / 1000) });
        }
        catch (e) { }
        return false;
    }
...
function getHeliusListenerStats() { try {
    return { ..._heliusListenerStats };
}
catch {
    return null;
} }
function resetHeliusListenerStats() { try {
    _heliusListenerStats.processed = 0;
    _heliusListenerStats.enriched = 0;
    _heliusListenerStats.skippedLowConfidence = 0;
    _heliusListenerStats.parseErrors = 0;
}
catch { } }
// Lazy import so project can run without ws installed when not used
async function startHeliusWebsocketListener(options) {
    if (!USE_WS) {
    ...
    const _backgroundSlotLookupsInFlight = new Set();
    // in-memory set to dedupe background per-mint officialEnrich resolutions
    const _backgroundMintEnrichInFlight = new Set();
        ...
        try {
            ...
            _diagInterval = setInterval(() => {
                try {
                    ...
                    else if ((s.processed || 0) % 500 === 0)
                        console.log('HELIUS-LISTENER-STATS summary:', JSON.stringify({ processed: s.processed, enriched: s.enriched, skippedLowConfidence: s.skippedLowConfidence }));
                }
                catch (e) { }
            }, interval);
        }
    ...
    ws.on('open', () => {
        ...
        startDiagInterval();
        try {
            startSlotBlocktimeWarmer({ intervalMs: Number(process.env.SLOT_BLOCKTIME_WARMER_INTERVAL_MS || 30000), lookbackSlots: Number(process.env.SLOT_BLOCKTIME_WARMER_LOOKBACK || 400) });
        }
        catch (e) { }
        try {
            ...
            const subscriptions = {};
            let sid = 1;
            const sendSub = (method, params, tag) => {
                const id = sid++;
        ...
        try {
            ...
            try {
                ...
                if (evt.eventType === 'memo') {
                    ...
                    const hasInnerWithPost = !!(v && v.meta && Array.isArray(v.meta.innerInstructions) && v.meta.innerInstructions.some((b) => Array.isArray(b.instructions) && b.instructions.some((ins) => ins && ins.parsed && ins.parsed.info && Array.isArray(ins.parsed.info.postTokenBalances) && ins.parsed.info.postTokenBalances.length > 0)));
                    ...
                    if (!hasPostBalances && !hasInnerWithPost) {
                        ...
                        if (VERBOSE)
                            console.log('Rejecting memo-only event (conservative policy): no on-chain corroboration', evt.mint || '(no-mint)');
                        return;
                    }
                }
                pushEvent(evt);
                try {
                    if (options?.onNewMint)
                        options.onNewMint(evt);
                }
                catch (e) { }
                        ...
                        if (quickHintFound) {
                            ...
                            if (evt.eventType === 'memo') {
                                ...
                                if (!hasPostBalances) {
                                    _heliusListenerStats.skippedLowConfidence = (_heliusListenerStats.skippedLowConfidence || 0) + 1;
                                    try {
                                        if ((_heliusListenerStats.skippedLowConfidence || 0) % 50 === 1)
                                            console.log('Skipping memo-only event (no postTokenBalances)', evt.mint || '(no-mint)');
                                    }
                                    catch { }
                                    return;
                                }
                            ...
                            try {
                                ...
                                if (!_backgroundMintEnrichInFlight.has(String(tokenAddr).toLowerCase())) {
                                    ...
                                    (async () => {
                                        try {
                                            const tokenObj = { tokenAddress: tokenAddr };
                                            ...
                                            try {
                                                _backgroundMintEnrichInFlight.delete(String(tokenAddr).toLowerCase());
                                            }
                                            catch (e) { }
                                            try {
                                                _heliusListenerStats.bgMintInFlight = Math.max(0, (_heliusListenerStats.bgMintInFlight || 1) - 1);
                                            }
                                            catch (e) { }
                                        }
                                    })();
                                ...
                                _heliusListenerStats.skippedLowConfidence = (_heliusListenerStats.skippedLowConfidence || 0) + 1;
                                try {
                                    if ((_heliusListenerStats.skippedLowConfidence || 0) % 10 === 1)
                                        console.log('Skipping enrichment due to low confidence (sampled)', evt.mint || '(no-mint)', 'score=', typeof score === 'number' && score.toFixed ? score.toFixed(2) : score);
                                }
                                catch { }
                                return;
                            }
                            ...
                            try {
                                ...
                                if (!txLikely) {
                                    ...
                                    try {
                                        if ((_heliusListenerStats.skippedLowConfidence || 0) % 50 === 1)
                                            console.log('Skipping low-confidence event: tx does not show token activity', evt.mint || '(no-mint)', 'eventType=', evt.eventType);
                                    }
                                    catch { }
                                    return;
                                }
                                ...
                                if (mintAddr) {
                                    try {
                                        if (HELIUS_REQUIRE_DEXSCREENER)
                                            isTraded = await quickIsTradedByDex(mintAddr);
                                        else
                                            isTraded = await quickIsTraded(mintAddr);
                                    }
                                    catch (e) {
                                        isTraded = false;
                                    }
                                ...
                                let isYoung = false;
                                try {
                                    isYoung = await quickIsMintYoung(evt.mint);
                                }
                                catch (e) {
                                    isYoung = false;
                                }
                                if (!isTraded || !isYoung) {
                                    ...
                                    try {
                                        if ((_heliusListenerStats.skippedLowConfidence || 0) % 50 === 1)
                                            console.log('Skipping enrichment due to low confidence (not traded/too-old)', evt.mint || '(no-mint)', 'score=', typeof score === 'number' && score.toFixed ? score.toFixed(2) : score);
                                    }
                                    catch { }
                                    return;
                                }
                                ...
                                _heliusListenerStats.skippedLowConfidence = (_heliusListenerStats.skippedLowConfidence || 0) + 1;
                                try {
                                    console.log('Skipping enrichment due to low confidence', evt.mint || '(no-mint)', 'score=', typeof score === 'number' && score.toFixed ? score.toFixed(2) : score);
                                }
                                catch { }
                                return;
                            }
                ...
                try {
                        ...
                        try {
                            if (_getCachedBlockTimeForSlot) {
                                ...
                                if (cached || cached === 0) {
                                    ...
                                    quickDebug.blockTimeDiag = { reason: blockTimeReason, elapsedMs: elapsed };
                                }
                                else {
                            ...
                            if ((blockTime === null || blockTime === undefined) && _getBlockTimeForSlotCached && ALLOW_SYNC) {
                                try {
                                    ...
                                    if (wrapped || wrapped === 0) {
                                        ...
                                        quickDebug ?? (quickDebug = {});
                                        quickDebug.blockTimeDiag = { reason: blockTimeReason, elapsedMs: elapsedSync };
                                    }
                                    else {
                                ...
                                (async () => {
                                    try {
                                        ...
                                        if (bt || bt === 0) {
                                            ...
                                            try {
                                                if (VERBOSE)
                                                    console.log('helius_quick_enrich_followup:', evt.mint || '(no-mint)', 'slot=', slotNum, 'blockTime=', bt, 'diag=', JSON.stringify(follow._diag));
                                            }
                                            catch { }
                                            if (VERBOSE)
                                                console.log(JSON.stringify({ helius_quick_enrich_followup: follow }));
                                            ...
                                            try {
                                                if (VERBOSE)
                                                    console.log('helius_quick_enrich_followup:', evt.mint || '(no-mint)', 'slot=', slotNum, 'blockTime= null', 'diag=', JSON.stringify(follow._diag));
                                            }
                                            catch { }
                                            if (VERBOSE)
                                                console.log(JSON.stringify({ helius_quick_enrich_followup: follow }));
                                        ...
                                        try {
                                            _backgroundSlotLookupsInFlight.delete(slotNum);
                                        }
                                        catch (e) { }
                                        try {
                                            _heliusListenerStats.bgInFlight = Math.max(0, (_heliusListenerStats.bgInFlight || 1) - 1);
                                        }
                                        catch (e) { }
                                    }
                                })();
                    ...
                    let mintSnapshot = null;
                    let mintSnapshotDiag = null;
                    try {
                        if (evt && evt.mint) {
                        ...
                        mintSnapshot = null;
                        mintSnapshotDiag = { reason: 'exception' };
                    }
                    const quick = {
                        mint: evt.mint || null,
                        eventType: evt.eventType || null,
                    ...
                    try {
                        if (mintSnapshot)
                            quick.mintSnapshot = mintSnapshot;
                        if (mintSnapshotDiag)
                            quick._diag = quick._diag || {};
                        quick._diag.mintSnapshotDiag = mintSnapshotDiag;
                    }
                    catch (e) { }
                    // best-effort: lift numeric snapshot fields into quick top-level for easier parsing/filters
                    try {
                        if (mintSnapshot) {
                            if (typeof mintSnapshot.approxLiquidity === 'number')
                                quick.approxLiquidity = mintSnapshot.approxLiquidity;
                            if (typeof mintSnapshot.volume_60s === 'number')
                                quick.volume_60s = mintSnapshot.volume_60s;
                            if (typeof mintSnapshot.volume_3600s === 'number')
                                quick.volume_3600s = mintSnapshot.volume_3600s;
                        }
                    }
                    catch (e) { }
                    ...
                    try {
                        if (!quickDebug)
                            quickDebug = {};
                        if (Object.keys(quickDebug).length)
                            quick._diag = quickDebug;
                    }
                    catch { }
                        ...
                        if ((!blockTime && blockTime !== 0) && typeof evt.mint === 'string' && evt.mint.length) {
                            try {
                                ...
                                if (quickTs && quickTs.ts) {
                                    ...
                                    quickDebug ?? (quickDebug = {});
                                    quickDebug.blockTimeDiag = { reason: blockTimeReason, source: quickTs.source || 'onchain', elapsedMs: null };
                                }
                            }
                            catch (e) { }
                        }
                    ...
                    try {
                        ...
                        if ((!blockTime && blockTime !== 0)) {
                            try {
                                ...
                                let isYoung = false;
                                try {
                                    if (String(evt.mint || '').trim())
                                        isTraded = await quickIsTraded(String(evt.mint).trim());
                                }
                                catch (e) {
                                    isTraded = false;
                                }
                                ...
                                if (!(isTraded && isYoung)) {
                                    ...
                                    try {
                                        if ((_heliusListenerStats.skippedLowConfidence || 0) % 50 === 1)
                                            console.log('Skipping quick-enrich: missing blockTime and not traded/young', evt.mint || '(no-mint)');
                                    }
                                    catch (e) { }
                        ...
                        if (!last || (nowSec - last) > effectiveDedupe) {
                            ...
                            try {
                                _recentAccepted.set(k, nowSec);
                            }
                            catch (e) { }
                            const diagShort = quick._diag ? `diag=${JSON.stringify(quick._diag)}` : '';
                            try {
                        ...
                        try {
                                            ...
                                            try {
                                                ...
                                                if (res && res.meta && Array.isArray(res.meta.postTokenBalances)) {
                                                    for (const b of res.meta.postTokenBalances) {
                                                        try {
                                                            if (b && b.mint === evt.mint && b.uiTokenAmount && b.uiTokenAmount.uiAmount) {
                                                                amt = Math.max(amt, Number(b.uiTokenAmount.uiAmount));
                                                            }
                                                        }
                                                        catch (e) { }
                                                    }
                            ...
                            if (p && typeof p.then === 'function') {
                                ...
                                p.then((res) => {
                                    try {
                                        if (res && res.skipped)
                                            console.log('Enrichment result skipped for', evt.mint || '(no-mint)', 'reason=', res.reason);
                                        else if (res)
                                            console.log('Enrichment completed for', evt.mint || '(no-mint)', safeString(res, 400));
                                    }
                                    catch (e) { }
                                }).catch((err) => { try {
                                    console.warn('Enrichment promise rejected', err && err.message ? err.message : err);
                                }
                                catch { } });
                            }
                        }
...
function analyzeHeliusMessage(parsed) {
    ...
    try {
        ...
        if (v && Array.isArray(v.logs))
            logs.push(...v.logs.filter((x) => typeof x === 'string'));
        if (v && v.meta && Array.isArray(v.meta.logMessages))
            logs.push(...v.meta.logMessages.filter((x) => typeof x === 'string'));
        ...
        if (logs.length) {
            for (const ln of logs) {
                ...
                if (low.includes('initialize_mint') || low.includes('create_metadata') || low.includes('create metadata account') || low.includes('create_metadata_account')) {
                    ...
                    if (m && m.length)
                        return { mint: m[0], eventType: 'metadata_or_initialize', detectedAt: new Date().toISOString(), detectedAtSec: Math.floor(Date.now() / 1000), firstSlot: slotFromCtx ?? null, raw: parsed };
                }
                // memo program may include a raw mint in memo text or label
                if (low.includes('memo') || ln.includes('Memo') || ln.includes('memo:')) {
                    const m = ln.match(/[1-9A-HJ-NP-Za-km-z]{32,44}/g);
                    if (m && m.length)
                        return { mint: m[0], eventType: 'memo', detectedAt: new Date().toISOString(), detectedAtSec: Math.floor(Date.now() / 1000), firstSlot: slotFromCtx ?? null, raw: parsed };
                }
                ...
                if (labelMatch && labelMatch[1])
                    return { mint: labelMatch[1], eventType: 'label', detectedAt: new Date().toISOString(), detectedAtSec: Math.floor(Date.now() / 1000), firstSlot: slotFromCtx ?? null, raw: parsed };
            }
        }
                    ...
                    try {
                        if (ins.parsed && typeof ins.parsed === 'object') {
                            ...
                            if (info && typeof info === 'object') {
                                ...
                                if (typeof cand === 'string' && cand.length >= 32 && cand.length <= 44)
                                    return { mint: cand, eventType: 'inner_parsed', detectedAt: new Date().toISOString(), detectedAtSec: Math.floor(Date.now() / 1000), firstSlot: slotFromCtx ?? null, raw: parsed };
                                ...
                                if (info.postTokenBalances && Array.isArray(info.postTokenBalances)) {
                                    for (const b of info.postTokenBalances) {
                                        if (b && typeof b.mint === 'string' && b.mint.length >= 32 && b.mint.length <= 44)
                                            return { mint: b.mint, eventType: 'postTokenBalance', detectedAt: new Date().toISOString(), detectedAtSec: Math.floor(Date.now() / 1000), firstSlot: slotFromCtx ?? null, raw: parsed };
                                    }
                                }
                            }
                        }
                ...
                try {
                    if (ins.parsed && typeof ins.parsed === 'object') {
                        ...
                        if (info && typeof info === 'object') {
                            const cand = info.mint || info.token || info.mintAddress || info.account || info.destination || info.source || null;
                            if (typeof cand === 'string' && cand.length >= 32 && cand.length <= 44)
                                return { mint: cand, eventType: 'parsed_instruction', detectedAt: new Date().toISOString(), detectedAtSec: Math.floor(Date.now() / 1000), firstSlot: slotFromCtx ?? null, raw: parsed };
                            if (info.postTokenBalances && Array.isArray(info.postTokenBalances)) {
                                for (const b of info.postTokenBalances) {
                                    if (b && typeof b.mint === 'string' && b.mint.length >= 32 && b.mint.length <= 44)
                                        return { mint: b.mint, eventType: 'postTokenBalance', detectedAt: new Date().toISOString(), detectedAtSec: Math.floor(Date.now() / 1000), firstSlot: slotFromCtx ?? null, raw: parsed };
    ...
    const found = {};
    function walk(o, depth = 0, parentKey) {
        if (!o || depth > 10)
            return;
        if (Array.isArray(o))
            return o.forEach((x) => walk(x, depth + 1, parentKey));
        if (typeof o === 'object') {
            for (const k of Object.keys(o)) {
                try {
                    ...
                    if ((k === 'mint' || k === 'token' || k === 'mintAddress' || k === 'account' || k === 'accounts') && typeof v2 === 'string' && v2.length >= 32 && v2.length <= 44) {
                        ...
                        if (!HELIUS_KNOWN_PROGRAM_IDS.has(v2) && !deny.has(lower))
                            found.mint = v2;
                    }
                    ...
                    if (k === 'postTokenBalances' && Array.isArray(v2)) {
                        for (const b of v2) {
                            try {
                                if (b && typeof b.mint === 'string' && b.mint.length >= 32 && b.mint.length <= 44 && !HELIUS_KNOWN_PROGRAM_IDS.has(b.mint)) {
                                    found.mint = b.mint;
                                    found.context = 'postTokenBalances';
                                }
                            }
                            catch (e) { }
                        }
                    ...
                    if (typeof v2 === 'string') {
                        ...
                        if (m && m.length) {
                            ...
                            const cand = m[0];
                            const deny = new Set(['11111111111111111111111111111111', 'tokenkegqfezyinwajbnbgkpfxcwubvf9ss623vq5da', 'memosq4gqabaxkb96qnh8tysncwxmywcqxgdlgmfchr', 'metaqbxxuerdq28cj1rbawkyqm3ybzjb6a8bt518x1s', 'so11111111111111111111111111111111111111112']);
                            if (deny.has(cand.toLowerCase()))
                                continue;
                            if (lower.includes('log') || lower.includes('message') || v2.toLowerCase().includes('mint')) {
                                if (!HELIUS_KNOWN_PROGRAM_IDS.has(cand)) {
                                    found.mint = found.mint || cand;
                                    found.context = 'log';
                                }
                            }
                            else {
                                ...
                                if (!HELIUS_KNOWN_PROGRAM_IDS.has(cand))
                                    found.candidate = found.candidate || cand;
                            }
                        }
                    }
                    walk(v2, depth + 1, k);
...
function computeEventConfidence(evt, raw) {
    try {
        if (!evt || typeof evt !== 'object')
            return 0;
        const t = evt.eventType || '';
        if (t === 'metadata_or_initialize')
            return 1.0;
        if (t === 'inner_parsed' || t === 'parsed_instruction' || t === 'postTokenBalance')
            return 0.94;
...
function isLikelyTokenByTx(parsed, evt) {
    try {
        if (!parsed || !evt)
            return false;
        const res = parsed.params?.result || parsed.result || parsed;
        ...
        try {
            ...
            if (Array.isArray(post) && post.length) {
                for (const b of post) {
                    try {
                        if (b && b.mint && String(b.mint) === String(evt.mint))
                            return true;
                    }
                    catch (e) { }
                }
                    ...
                    for (const ins of block.instructions) {
                        try {
                            if (ins.parsed && typeof ins.parsed === 'object') {
                                ...
                                if (/initializemint|mintto|transfer|create_metadata|create_metadata_account/.test(rawType))
                                    return true;
                                if (info && info.mint && String(info.mint) === String(evt.mint))
                                    return true;
        ...
        try {
            ...
            if (Array.isArray(v?.logs))
                logs.push(...v.logs.filter((x) => typeof x === 'string'));
            if (Array.isArray(v?.meta?.logMessages))
                logs.push(...v.meta.logMessages.filter((x) => typeof x === 'string'));
            for (const ln of logs) {
                try {
                    const low = (ln || '').toLowerCase();
                    if (low.includes('create_metadata') || low.includes('initialize_mint') || low.includes('mint_to') || low.includes('create_metadata_account'))
                        return true;
                    // if log contains mint address specifically
                    const m = (ln || '').match(/[1-9A-HJ-NP-Za-km-z]{32,44}/g);
                    if (m && m.length && m[0] === String(evt.mint))
                        return true;
                }
                catch (e) { }
            }
...
function saveLastInstance(inst) { _lastInstance = inst; }
function getRecentHeliusEvents() { try {
    return _lastInstance ? _lastInstance.getRecentEvents() : [];
}
catch {
    return [];
} }
...
if (require.main === module) {
    (async () => {
        const instance = await startHeliusWebsocketListener({
            ...
            onMessage: (m) => { try {
                const s = typeof m === 'string' ? m.slice(0, 120) : JSON.stringify(m || '').slice(0, 120);
                console.log('WS message sample:', s);
            }
            catch (e) {
                console.log('WS message sample: <unserializable>');
            } },
