...
exports.welcomeGuideHandler = exports.welcomeScreenHandler = exports.welcomeKeyboardList = void 0;
let TelegramBot;
try {
...
const welcomeScreenHandler = async (bot, msg) => {
    try {
        const { username, id: chat_id, first_name, last_name } = msg.chat;
        // check if bot
        if (!username) {
            bot.sendMessage(chat_id, "⚠️ You have no telegram username. Please take at least one and try it again.");
            return;
        }
        ...
        // send welcome guide
        await (0, exports.welcomeGuideHandler)(bot, msg);
...
exports.welcomeScreenHandler = welcomeScreenHandler;
const newUserHandler = async (bot, msg) => {
    const { username, id: chat_id, first_name, last_name } = msg.chat;
    let retries = 0;
    let userdata = null;
    let private_key = "";
...
const welcomeGuideHandler = async (bot, msg, replaceId) => {
    ...
    const reply_markup = {
        inline_keyboard: exports.welcomeKeyboardList.map((rowItem) => rowItem.map((item) => {
...
exports.welcomeGuideHandler = welcomeGuideHandler;
