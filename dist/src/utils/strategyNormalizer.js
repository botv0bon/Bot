 ...
 */
function normalizeStrategy(orig) {
    const s = Object.assign({}, orig || {});
    // map synonyms
    if (s.profitTargetPercent && !s.target1)
        s.target1 = s.profitTargetPercent;
    if (s.targetPercent && !s.target1)
        s.target1 = s.targetPercent;
    if (s.sellPercent && !s.sellPercent1)
        s.sellPercent1 = s.sellPercent;
