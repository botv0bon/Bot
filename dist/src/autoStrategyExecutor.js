...
exports.autoExecuteStrategyForUser = autoExecuteStrategyForUser;
 ...
 */
async function autoExecuteStrategyForUser(user, tokens, mode = 'buy') {
    if (!user.strategy || !user.wallet || !user.secret || user.strategy.enabled === false)
        return;
    ...
    for (const token of tokensToTrade) {
        try {
            ...
            if (mode === 'buy') {
                result = await (0, tradeSources_1.unifiedBuy)(tokenAddress, buyAmount, user.secret);
            }
            else {
                ...
                result = await (0, tradeSources_1.unifiedSell)(tokenAddress, sellAmount, user.secret);
            }
