...
exports.SCRIPTS_DENY = exports.SCRIPTS_RULES = void 0;
exports.hashTokenAddress = hashTokenAddress;
exports.readSentHashes = readSentHashes;
exports.appendSentHash = appendSentHash;
exports.startFastTokenFetcher = startFastTokenFetcher;
exports.getUnifiedCandidates = getUnifiedCandidates;
exports.fetchLatestWithSources = fetchLatestWithSources;
exports.fetchAndFilterTokensForUsers = fetchAndFilterTokensForUsers;
exports.getGlobalFetchCache = getGlobalFetchCache;
exports.ensureCanonicalOnchainAges = ensureCanonicalOnchainAges;
exports.analyzeFetchSources = analyzeFetchSources;
exports.runDeepCacheCheck = runDeepCacheCheck;
exports.captureHeliusAndVerify = captureHeliusAndVerify;
exports.heliusGetSignaturesFast = heliusGetSignaturesFast;
exports.mintPreviouslySeen = mintPreviouslySeen;
exports.getMintSnapshotCached = getMintSnapshotCached;
exports.updateMintStatsFromEvent = updateMintStatsFromEvent;
exports.getBlockTimeForSlotCached = getBlockTimeForSlotCached;
exports.getCachedBlockTimeForSlot = getCachedBlockTimeForSlot;
exports.getAccountInfo = getAccountInfo;
exports.handleNewMintEvent = handleNewMintEvent;
exports.handleNewMintEventCached = handleNewMintEventCached;
exports.runFastDiscoveryCli = runFastDiscoveryCli;
exports.fetchLatest5FromAllSources = fetchLatest5FromAllSources;
exports.testFetchEnrichFilterLatest = testFetchEnrichFilterLatest;
const axios_1 = __importDefault(require("axios"));
const web3_js_1 = require("@solana/web3.js");
const strategy_1 = require("./bot/strategy");
const redis_1 = require("redis");
const strategyNormalizer_1 = require("./utils/strategyNormalizer");
const strategy_2 = require("./bot/strategy");
...
const KNOWN_NON_MINT_ADDRESSES = new Set([
...
const enrichHelpers_1 = require("./utils/enrichHelpers");
const sourceEquivalence_1 = require("./utils/sourceEquivalence");
...
async function isProcessedGlobal(key) {
    try {
...
async function markProcessedGlobal(key, ttlSec = 60) {
    try {
        const c = await getRedisClient();
        if (!c)
            return false;
...
function hashTokenAddress(addr) {
    return String(addr || '').toLowerCase().trim();
}
async function ensureSentDir() {
    const sent = path_1.default.join(process.cwd(), 'sent_tokens');
    try {
...
async function readSentHashes(userId) {
    try {
        ...
        const file = path_1.default.join(sentDir, `${userId}.json`);
...
async function appendSentHash(userId, hash) {
    try {
        ...
        try {
            ...
            if (rc) {
                ...
                try {
                    ...
                    if (ttl > 0)
                        await rc.expire(key, Math.max(1, Math.floor(ttl))).catch(() => { });
                }
                catch (e) { }
                return;
            }
        ...
        await (0, helpers_1.writeJsonFile)(file, arr.slice(-500)).catch(() => { });
    }
    catch (e) { }
}
/**
 ...
 */
function startFastTokenFetcher(users, telegram, options) {
    const intervalMs = options?.intervalMs || 1000;
    const perUserLimit = options?.perUserLimit || 1; // max messages per interval
    const batchLimit = options?.batchLimit || 20; // tokens per user per interval
    let running = true;
        ...
        while (running) {
            try {
                ...
                for (const uid of processOrder) {
                    try {
                        const u = users[uid];
                        if (!u || !u.strategy || u.strategy.enabled === false)
                            continue;
                        ...
                        if (u.strategy.autoBuy !== false && Number(u.strategy.buyAmount) > 0) {
                            for (const token of filtered) {
                                const addr = token.tokenAddress || token.address || token.mint || token.pairAddress || '';
                                try {
                                    const finalStrategy = (0, strategyNormalizer_1.normalizeStrategy)(u.strategy);
                                    ...
                                    const result = await (0, tradeSources_1.unifiedBuy)(addr, amount, u.secret);
                                    if (result && (result.tx || result.success)) {
                                        try {
                                            await (0, strategy_2.registerBuyWithTarget)(u, { address: addr, price: token.price || token.priceUsd }, result, u.strategy.targetPercent || 10);
                                        }
                                        catch { }
                                        const { fee, slippage } = (0, tradeMeta_1.extractTradeMeta)(result, 'buy');
                                        u.history = u.history || [];
                                        ...
                                        try {
                                            await telegram.sendMessage(uid, `✅ AutoBuy executed for ${addr}\nTx: ${resTx}`);
                                        }
                                        catch { }
                                    }
                                }
...
// Defensive helpers
function safeStringSnippet(v, n = 200) {
    try {
...
function mergeAndCanonicalizeCache(arr) {
    try {
        ...
        for (const raw of arr || []) {
            try {
                ...
                for (const k of preferList) {
                    if (!existing[k] && raw[k])
                        existing[k] = raw[k];
                    else if (!existing[k] && raw.token && raw.token[k])
                        existing[k] = raw.token[k];
                }
                ...
                for (const k of fieldsToCoerce) {
                    ...
                    if (typeof v2 === 'string' && !isNaN(Number(v2)))
                        v2 = Number(v2);
                    if (typeof v1 === 'number' && typeof v2 === 'number')
                        existing[k] = Math.max(v1, v2);
                    else if ((v1 === undefined || v1 === null) && (v2 !== undefined && v2 !== null))
                        existing[k] = v2;
                }
                ...
                if (raw.sourceTags) {
                    ...
                    for (const s of incoming)
                        if (s && !existing.sourceTags.includes(s))
                            existing.sourceTags.push(s);
                }
                ...
                if (raw.__sources) {
                    ...
                    for (const src of incoming) {
                        try {
                            ...
                            const dup = existing.__sources.find((s) => { try {
                                return (s && (s.name || s.source || s.host || s.type)) === (src && (src.name || src.source || src.host || src.type));
                            }
                            catch (e) {
                                return false;
                            } });
                            if (!dup)
                                existing.__sources.push(src);
                        }
                        catch (e) { }
                    }
                }
                // try to set poolOpenTimeMs/ageSeconds from known aliases
                if (!existing.poolOpenTimeMs)
                    existing.poolOpenTimeMs = raw.poolOpenTimeMs || raw.poolOpenTime || (raw.token && raw.token.poolOpenTime) || null;
                if (!existing.ageSeconds)
                    existing.ageSeconds = raw.ageSeconds || (existing.ageMinutes ? existing.ageMinutes * 60 : null) || null;
                map[addr] = existing;
            }
            catch (e) {
                continue;
        ...
        for (const e of out) {
            try {
                ...
                if (e.poolOpenTimeMs && typeof e.poolOpenTimeMs === 'number' && !isNaN(e.poolOpenTimeMs)) {
                    ...
                    if (secs === undefined)
                        secs = Math.floor((Date.now() / 1000) - v);
                }
                ...
                if ((secs === undefined || isNaN(secs)) && typeof e.ageSeconds === 'number')
                    secs = Number(e.ageSeconds);
                if ((secs === undefined || isNaN(secs)) && typeof e.ageMinutes === 'number')
                    secs = Math.floor(Number(e.ageMinutes) * 60);
...
async function getUnifiedCandidates(limit) {
    const candidates = [];
    const seen = new Set();
    try {
        ...
        try {
            ...
            if (Array.isArray(evs)) {
                ...
                for (const e of evs) {
                    ...
                    const m = raw ? (0, tokenUtils_1.normalizeMintCandidate)(raw) : null;
                    // Skip obviously non-mint addresses and very-old events to reduce noise
                    if (!m)
                        continue;
                    if (KNOWN_NON_MINT_ADDRESSES.has(m))
                        continue;
                    ...
                    if (evtAge && now - Number(evtAge) > (24 * 3600))
                        continue;
                    if (m && !seen.has(m)) {
                        // attach structured provenance so Helius contributes to __sources in canonical cache
                        try {
                            const meta = (0, enrichHelpers_1.makeSourceMeta)('helius-ws', true, { raw: e });
        ...
        try {
            ...
            let data = [];
            const dexMeta = raw && raw.__meta ? raw.__meta : null;
            if (raw)
                data = Array.isArray(raw) ? raw : (raw.data || raw.pairs || raw.items || []);
            for (const it of data) {
                const rawMint = extractMintFromItemLocal(it);
                const m = rawMint ? (0, tokenUtils_1.normalizeMintCandidate)(rawMint) : null;
                if (m && !seen.has(m)) {
                    const obj = { mint: m };
                    if (dexMeta)
                        obj.__sources = [dexMeta];
    ...
    return uniq.map((m) => {
        ...
        if (typeof m === 'string')
            return { mint: m };
        if (typeof m === 'object' && m.mint)
            return { mint: m.mint, __sources: m.__sources || null, sourceTags: m.sourceTags || null };
        return { mint: String(m) };
    });
}
// Return a list of candidates with source tags for diagnostics
async function fetchLatestWithSources(limit = 50) {
    const out = [];
    try {
        const { getRecentHeliusEvents } = require('./heliusWsListener');
        const evs = getRecentHeliusEvents() || [];
        for (const e of evs) {
            const raw = e && (e.mint || (e.parsed && e.parsed.info && e.parsed.info.mint) || e.address || null);
            const m = (0, tokenUtils_1.normalizeMintCandidate)(raw);
            if (m)
                out.push({ mint: m, source: 'helius-ws' });
            if (out.length >= limit)
                break;
        }
    }
    ...
    try {
        ...
        if (raw)
            data = Array.isArray(raw) ? raw : (raw.data || raw.pairs || raw.items || []);
        for (const it of data) {
            const m = (0, tokenUtils_1.normalizeMintCandidate)(extractMintFromItemLocal(it));
            if (m) {
                const entry = { mint: m, source: 'dexscreener' };
                if (dexMeta)
                    entry.__meta = dexMeta;
                out.push(entry);
            }
            if (out.length >= limit)
                break;
    ...
    try {
        const subs = await fetchLatest5FromAllSources(Math.max(5, limit));
        for (const m of subs.heliusEvents || [])
            if (m)
                out.push({ mint: m, source: 'helius-history' });
        for (const m of subs.dexTop || [])
            if (m)
                out.push({ mint: m, source: 'dex-top' });
    }
    catch (e) { }
 ...
 */
async function fetchAndFilterTokensForUsers(users, opts) {
    const now = Date.now();
    if (!opts?.force && __global_fetch_cache.length && (now - __global_fetch_ts) < __GLOBAL_FETCH_TTL) {
            ...
            if (coldEnabled) {
                try {
                    ...
                    if (Array.isArray(seedCandidates) && seedCandidates.length) {
                        ...
                        const runner = (0, enrichHelpers_1.createLimiter)(Math.max(1, lim));
                        await Promise.all(seedCandidates.slice(0, seedLimit).map((c) => runner(async () => {
                            try {
                                if (typeof globalThis.__fastTokenFetcher_getAccountInfo === 'function')
                                    return await globalThis.__fastTokenFetcher_getAccountInfo(c.mint || c);
                                // fallback to local exported getAccountInfo if available
                                if (typeof (require('./src/fastTokenFetcher').getAccountInfo) === 'function')
                                    return await require('./src/fastTokenFetcher').getAccountInfo(c.mint || c);
                            }
                            catch (e) { }
                        })));
                    }
        ...
        try {
            ...
            __global_fetch_cache = await getUnifiedCandidates(limit).catch(async (e) => {
                try {
                    const { fetchUnifiedTokens } = require('./utils/tokenUtils');
                    return await fetchUnifiedTokens('solana', { limit: String(limit) });
                }
                catch (ee) {
                    return [];
                }
            ...
            try {
                const tu = require('./utils/tokenUtils');
                __global_fetch_cache = (__global_fetch_cache || []).map((t) => {
                    try {
            ...
            try {
                const tu = require('./utils/tokenUtils');
                try {
                    const ds = await (0, tokenUtils_1.fetchDexScreenerTokens)('solana', { limit: String(limit) });
                        ...
                        for (const d of ds) {
                            try {
                                ...
                                if (existing) {
                                    ...
                                    // merge source tags
                                    existing.sourceTags = Array.isArray(existing.sourceTags) ? existing.sourceTags : (existing.sourceTags ? [existing.sourceTags] : []);
                                    if (!existing.sourceTags.includes('dexscreener'))
                                        existing.sourceTags.push('dexscreener');
                                    // attach structured source meta if present
                                    try {
                                        const meta = (d && (d.__meta || d.meta)) ? (d.__meta || d.meta) : null;
                                        existing.__sources = Array.isArray(existing.__sources) ? existing.__sources : (existing.__sources ? [existing.__sources] : []);
                                        if (meta)
                                            existing.__sources.push(meta);
                                    }
                                    catch (e) { }
                                    ...
                                    try {
                                        if (d && d.__meta)
                                            newEntry.__sources = [d.__meta];
                                    }
                                    catch (e) { }
                                    newEntry.poolOpenTimeMs = d.poolOpenTimeMs || d.poolOpenTime || null;
                                    if (newEntry.poolOpenTimeMs && typeof newEntry.poolOpenTimeMs === 'number' && newEntry.poolOpenTimeMs < 1e12 && newEntry.poolOpenTimeMs > 1e9)
                                        newEntry.poolOpenTimeMs *= 1000;
                ...
                try {
                    try {
                        const eq = (0, sourceEquivalence_1.compareSourcesForCache)(__global_fetch_cache || []);
                        (0, sourceEquivalence_1.printEquivalenceReport)(eq);
                    }
                    catch (e) { }
                    // Populate canonical on-chain ages (best-effort, bounded concurrency)
                    try {
                        await ensureCanonicalOnchainAges(__global_fetch_cache || [], { timeoutMs: Number(process.env.ONCHAIN_FRESHNESS_TIMEOUT_MS || 3000), concurrency: Number(process.env.ONCHAIN_FILL_CONCURRENCY || 3) });
                    }
                    catch (e) { }
                }
                catch (e) { }
            ...
            try {
                ...
                const limiter = (0, enrichHelpers_1.getHostLimiter)(host, Number(process.env.COINGECKO_CONCURRENCY || 1));
                const startCg = Date.now();
                try {
                    const cg = await limiter(async () => await (0, enrichHelpers_1.retryWithBackoff)(() => (0, tokenUtils_1.fetchSolanaFromCoinGecko)(), { retries: Number(process.env.COINGECKO_RETRIES || 1), baseMs: 200 }));
                    if (cg) {
                        // attach meta for source tracing
                        const meta = (0, enrichHelpers_1.makeSourceMeta)('coingecko', true, { latencyMs: Date.now() - startCg, raw: cg });
                        if (cg && typeof cg === 'object')
                            cg.__sources = Array.isArray(cg.__sources) ? cg.__sources.concat(meta) : [meta];
    ...
    for (const uid of Object.keys(users)) {
        ...
        if (!u || !u.strategy)
            continue;
        try {
            normalizedMap[uid] = (0, strategyNormalizer_1.normalizeStrategy)(u.strategy);
        }
        catch {
    ...
    for (const uid of Object.keys(normalizedMap)) {
        ...
        try {
                ...
                async function worker() {
                    while (idx < sample.length) {
                        ...
                        try {
                            ...
                            const limiter = (0, enrichHelpers_1.getHostLimiter)(host, Number(process.env.FASTFETCH_ENRICH_CONCURRENCY || 1));
                            const startE = Date.now();
                            try {
                                await limiter(async () => {
                                    const wrap = await (0, enrichHelpers_1.withTimeout)(officialEnrich(t, { amountUsd: Number(strat.buyAmount) || 50, timeoutMs: 4000 }), 4500);
                                    if (!wrap.ok)
                                        throw new Error(String(wrap.error || 'timeout'));
                                });
                                t.__sources = Array.isArray(t.__sources) ? t.__sources : (t.__sources ? [t.__sources] : []);
                                t.__sources.push((0, enrichHelpers_1.makeSourceMeta)('officialEnrich', true, { latencyMs: Date.now() - startE }));
                            }
                            catch (ee) {
                                t.__sources = Array.isArray(t.__sources) ? t.__sources : (t.__sources ? [t.__sources] : []);
                                t.__sources.push((0, enrichHelpers_1.makeSourceMeta)('officialEnrich', false, { error: (ee && ee.message) || String(ee), latencyMs: Date.now() - startE }));
                            }
                        }
                        catch (e) { }
                    }
            ...
            try {
                workCache = mergeAndCanonicalizeCache(workCache || []);
            }
            catch (e) { }
                ...
                try {
                    ...
                    for (const m of matches) {
                        try {
                            ...
                            if (canon && canon.__sources)
                                merged.__sources = Array.isArray(canon.__sources) ? canon.__sources : [canon.__sources];
                            if (canon && canon._canonicalAgeSeconds !== undefined)
                                merged._canonicalAgeSeconds = canon._canonicalAgeSeconds;
...
async function ensureCanonicalOnchainAges(cache, opts) {
    ...
    try {
        ...
        const limiter = (0, enrichHelpers_1.createLimiter)(Math.max(1, concurrency));
        let idx = 0;
        const work = Array.from({ length: Math.min(cache.length, Math.max(3, concurrency)) }).map(() => (async () => {
            while (true) {
                ...
                try {
                    ...
                    if (e._canonicalAgeSeconds !== undefined && e._canonicalAgeSeconds !== null)
                        continue;
                    const addr = e.tokenAddress || e.address || e.mint || e.pairAddress;
                    if (!addr) {
                        e._canonicalAgeSeconds = e._canonicalAgeSeconds === undefined ? null : e._canonicalAgeSeconds;
                        continue;
                    }
                    // Bound the external call by limiter + timeout
                    await limiter(async () => {
                        try {
                            const wrapped = await (0, enrichHelpers_1.withTimeout)(tu.getFirstOnchainTimestamp(addr, { timeoutMs }), timeoutMs + 200);
                            if (!wrapped.ok) {
                                // mark as missing so we don't retry aggressively
                                e._canonicalAgeSeconds = null;
                                return;
                            }
                            ...
                            if (ts) {
                                ...
                                try {
                                    ...
                                    e.__sources.push((0, enrichHelpers_1.makeSourceMeta)('first-onchain', true, { raw: { source: res && res.source ? res.source : 'onchain', cached: !!res?.cached } }));
                                }
                                catch (ignored) { }
                            }
                            else {
...
function analyzeFetchSources() {
    const cache = getGlobalFetchCache() || [];
    const perTokenCounts = [];
    const globalSourceSet = new Set();
    for (const it of cache) {
        try {
            ...
            for (const t of tags) {
                try {
                    normalized.push(String(t));
                    globalSourceSet.add(String(t));
                }
                catch (e) { }
            }
...
async function runDeepCacheCheck(opts) {
    ...
    try {
        ...
        const mapped = (global || []).map((g) => ({ addr: g.tokenAddress || g.address || g.mint || '(no-addr)', poolOpenTimeMs: g.poolOpenTimeMs || g.firstSeenAtMs || null })).filter((x) => x.poolOpenTimeMs).map((x) => ({ ...x, ageMin: (now - Number(x.poolOpenTimeMs)) / 60000 })).sort((a, b) => a.poolOpenTimeMs - b.poolOpenTimeMs);
        console.log('global cache size:', (global || []).length);
        if (!mapped.length) {
            console.log('no entries with poolOpenTimeMs/firstSeenAtMs found in cache');
            return;
        }
        const newest = safeArraySlice(mapped, Math.max(0, (mapped || []).length - 40)).reverse();
        console.log('Newest entries (up to 40):');
        for (const it of safeArraySlice(newest, 0, 40))
            console.log(`- ${it.addr}  poolOpenTimeMs=${it.poolOpenTimeMs}  ageMin=${(it.ageMin || 0).toFixed(2)}`);
        const recent = safeArraySlice(newest.filter((x) => (x && typeof x.ageMin === 'number' ? x.ageMin : -1) >= 0 && (x && typeof x.ageMin === 'number' ? x.ageMin : -1) <= windowMin), 0, 5);
        console.log('\nentries with age <=', windowMin, 'minutes:', recent.length);
        if (!recent.length) {
            console.log(`No tokens <=${windowMin}min found; increase window or capture live events.`);
            return;
        }
        for (const it of recent) {
            ...
            try {
                const acct = await getAccountInfo(item.addr);
                console.log('getAccountInfo ok:', !!(acct && acct.value));
            }
            catch (e) {
                console.log('getAccountInfo err', String(e));
            }
            try {
                const sigs = await heliusGetSignaturesFast(item.addr, process.env.HELIUS_FAST_RPC_URL || config_1.HELIUS_RPC_URL || '', 4000, Number(process.env.HELIUS_RETRIES || 0));
                console.log('signatures:', Array.isArray(sigs) ? `count=${sigs.length}` : safeStringSnippet(sigs, 200));
            }
            catch (e) {
                console.log('sigs err', String(e));
            }
        }
    }
    catch (e) {
        console.error('runDeepCacheCheck error', e && e.message ? e.message : e);
    }
}
// Start Helius WS listener for durationMs milliseconds, then run getAccountInfo/helius signatures for captured events
async function captureHeliusAndVerify(durationMs = 60000) {
    try {
        const wsMod = require('./heliusWsListener');
        console.log('Starting Helius WS listener for', durationMs, 'ms...');
        const inst = await (wsMod.startHeliusWebsocketListener ? wsMod.startHeliusWebsocketListener({ onOpen: () => console.log('WS open'), onMessage: () => { }, onClose: () => console.log('WS closed'), onError: (e) => console.warn('WS error', e && e.message) }) : null);
        ...
        for (let i = 0; i < limit; i++) {
            ...
            try {
                const acct = await getAccountInfo(mint);
                console.log('getAccountInfo.ok=', !!(acct && acct.value));
            }
            catch (err) {
                console.log('getAccountInfo.err', String(err));
            }
            try {
                const sigs = await heliusGetSignaturesFast(mint, process.env.HELIUS_FAST_RPC_URL || config_1.HELIUS_RPC_URL || '', 4000, Number(process.env.HELIUS_RETRIES || 1));
                if (!sigs)
                    console.log('signatures: null');
                else if (Array.isArray(sigs))
                    console.log('signatures count:', sigs.length, 'sample0:', safeStringSnippet(sigs[0], 200));
                else if (sigs.result && Array.isArray(sigs.result))
                    console.log('signatures count:', sigs.result.length, 'sample0:', safeStringSnippet(sigs.result[0], 200));
                else
                    console.log('signatures:', safeStringSnippet(sigs, 200));
            }
            catch (err) {
                console.log('sigs.err', String(err));
            }
        }
        try {
            if (inst && inst.stop)
        ...
        console.error('captureHeliusAndVerify err', e && e.message ? e.message : e);
    }
}
// =========================
...
async function fetchDexBoostsRaw(timeout = 3000) {
    ...
    try {
        ...
        const limiter = (0, enrichHelpers_1.getHostLimiter)(host, Number(process.env.DEXSCREENER_CONCURRENCY || 2));
        const res = await limiter(async () => {
            return await (0, enrichHelpers_1.retryWithBackoff)(async () => {
                const p = axios_1.default.get(url, { timeout });
                const wrap = await (0, enrichHelpers_1.withTimeout)(p, timeout + 200);
                if (!wrap.ok)
                    throw Object.assign(new Error(String(wrap.error || 'timeout')), { __metaErr: wrap.error });
                return wrap.result;
            }, { retries: Number(process.env.DEXSCREENER_RETRIES || 2), baseMs: 200 });
        });
        return { __meta: (0, enrichHelpers_1.makeSourceMeta)('dexscreener', true, { latencyMs: Date.now() - start, raw: res.data }), data: res.data };
    }
    catch (e) {
        const errMsg = e && (e.__metaErr || e.message || String(e)) || 'error';
        return { __meta: (0, enrichHelpers_1.makeSourceMeta)('dexscreener', false, { error: errMsg, latencyMs: Date.now() - start }) };
    }
}
function extractMintFromItemLocal(it) {
    if (!it)
        return null;
    if (it.token && typeof it.token === 'object') {
        const t = it.token;
        return (0, tokenUtils_1.normalizeMintCandidate)(t.address || t.mint || null);
    }
    if (it.pair && typeof it.pair === 'object') {
        const p = it.pair;
        if (p.token && typeof p.token === 'object')
            return (0, tokenUtils_1.normalizeMintCandidate)(p.token.mint || p.token.address || null);
        if (p.baseToken && typeof p.baseToken === 'object')
            return (0, tokenUtils_1.normalizeMintCandidate)(p.baseToken.mint || p.baseToken.address || null);
        if (p.base && typeof p.base === 'object')
            return (0, tokenUtils_1.normalizeMintCandidate)(p.base.mint || p.base.address || null);
    }
    return (0, tokenUtils_1.normalizeMintCandidate)(it.tokenAddress || it.mint || it.address || null);
}
...
async function heliusGetSignaturesFast(mint, heliusUrl, timeout = 2500, retries = 0) {
    // simple in-memory cache to avoid repeated signature queries within a short window
    try {
        globalThis.__heliusSigCache = globalThis.__heliusSigCache || new enrichHelpers_1.TTLCache(Number(process.env.HELIUS_SIG_CACHE_MS || 30000));
        const sigCache = globalThis.__heliusSigCache;
    ...
    while (attempt <= retries) {
        try {
            ...
            try {
                globalThis.__RPC_STATS = globalThis.__RPC_STATS || { calls: 0, errors: 0, rateLimit429: 0, totalLatencyMs: 0 };
            }
            catch (e) { }
            const _stats = globalThis.__RPC_STATS || null;
            const host = (() => { try {
                return new URL(heliusUrl).host;
            }
            catch {
                return heliusUrl;
            } })();
            const limiter = (0, enrichHelpers_1.getHostLimiter)(host, Number(process.env.HELIUS_CONCURRENCY || 2));
            const start = Date.now();
            if (_stats)
                _stats.calls = (_stats.calls || 0) + 1;
            const res = await limiter(async () => {
                return await (0, enrichHelpers_1.retryWithBackoff)(async () => {
                    const headers = { 'Content-Type': 'application/json' };
                    try {
                        const _hk = (0, config_1.getHeliusApiKey)();
                        if (_hk)
                            headers['x-api-key'] = _hk;
                    }
                    catch (e) { }
                    const p = axios_1.default.post(heliusUrl, payload, { headers, timeout });
                    const wrap = await (0, enrichHelpers_1.withTimeout)(p, timeout + 200);
                    if (!wrap.ok)
                        throw Object.assign(new Error(String(wrap.error || 'timeout')), { __metaErr: wrap.error });
                    return wrap.result;
            ...
            try {
                if (_stats)
                    _stats.totalLatencyMs = (_stats.totalLatencyMs || 0) + latency;
            }
            catch (e) { }
            const resData = res;
            ...
            try {
                ...
                if (_stats) {
                    ...
                    if (status === 429)
                        _stats.rateLimit429 = (_stats.rateLimit429 || 0) + 1;
                }
            }
            catch (ee) { }
...
async function heliusRpc(method, params = [], timeout = 4000, retries = 0) {
    const heliusUrl = config_1.HELIUS_RPC_URL || process.env.HELIUS_FAST_RPC_URL;
    if (!heliusUrl)
        return { __error: 'no-helius-url' };
    // host cooldown guard (global)
    const hostStateKey = (() => { try {
        return new URL(heliusUrl).host;
    }
    catch {
        return heliusUrl;
    } })();
    if (globalThis.__hostCooldowns && globalThis.__hostCooldowns[hostStateKey] && Date.now() < globalThis.__hostCooldowns[hostStateKey].cooldownUntil) {
        ...
        try {
            ...
            if (status && (status === 429 || (status >= 500 && status < 600))) {
                ...
                try {
                    ...
                    globalThis.__hostCooldowns = globalThis.__hostCooldowns || {};
                    const st = globalThis.__hostCooldowns[host] = globalThis.__hostCooldowns[host] || { recent429: 0, cooldownUntil: 0 };
                    st.recent429 = (st.recent429 || 0) + 1;
                    const base = 1000;
                    const backoffMs = Math.min(60000, base * Math.pow(2, Math.min(6, st.recent429)) + Math.floor(Math.random() * 500));
...
async function mintPreviouslySeen(mint, txBlockTime, currentSig) {
    try {
        if (!mint)
            return null;
        const heliusUrl = process.env.HELIUS_FAST_RPC_URL || config_1.HELIUS_RPC_URL;
        if (!heliusUrl)
            return null;
        ...
        const sigs = arr.slice(0, lookback).map((x) => x?.signature || x?.txHash || x?.tx_hash).filter(Boolean);
        if (!sigs.length)
            return null;
...
function instructionReferencesMint(instrs, mint) {
    try {
        if (!instrs || !Array.isArray(instrs) || !mint)
            return false;
...
async function getParsedTransaction(signature) {
    ...
    try {
        ...
        if (heliusRes && !heliusRes.__error) {
...
async function getParsedTransactionViaConnection(signature) {
    try {
        const cfg = require('./config');
        const conn = cfg && cfg.connection;
        if (!conn || typeof conn.getParsedTransaction !== 'function')
            return { __error: 'no-connection' };
...
async function getBlockTimeForSlotCached(slot) {
    try {
        ...
        try {
            ...
            if (conn && typeof conn.getBlockTime === 'function') {
                ...
                if (bt || bt === 0) {
                    try {
                        if (rc)
                            await rc.setEx(key, Math.max(1, Math.floor(Number(process.env.SLOT_BLOCKTIME_TTL_SEC || 60 * 60 * 24 * 7))), String(bt)).catch(() => { });
                    }
                    catch (e) { }
                    return Number(bt);
                }
        ...
        try {
            ...
            if (mainRpc) {
                ...
                const r = await axios_1.default.post(mainRpc, payload, { timeout: 4000 });
                const val = r && r.data ? (r.data.result ?? r.data) : null;
                if (val || val === 0) {
                    const num = Number(val);
                    try {
                        if (rc)
                            await rc.setEx(key, Math.max(1, Math.floor(Number(process.env.SLOT_BLOCKTIME_TTL_SEC || 60 * 60 * 24 * 7))), String(num)).catch(() => { });
                    }
                    catch (e) { }
                    return num;
                }
...
async function getCachedBlockTimeForSlot(slot) {
    try {
        const rc = await getRedisClient().catch(() => null);
        if (!rc)
            return null;
 ...
 */
async function getMintSnapshotCached(mint) {
    try {
        if (!mint)
            return null;
        const rc = await getRedisClient().catch(() => null);
        ...
        if (rc) {
            ...
            const out = {};
            if (h.lastSeenTs)
                out.lastSeenTs = Number(h.lastSeenTs);
            if (out.lastSeenTs)
                out.ageSeconds = Math.max(0, Math.floor(Date.now() / 1000) - Number(out.lastSeenTs));
            if (h.volume_60s)
                out.volume_60s = Number(h.volume_60s);
            if (h.volume_3600s)
                out.volume_3600s = Number(h.volume_3600s);
            if (h.approxLiquidity)
                out.approxLiquidity = Number(h.approxLiquidity);
            if (h.eventCount)
                out.eventCount = Number(h.eventCount);
            return out;
        }
        // In-memory fallback
        try {
            // lazy-initialize in-memory store
            if (!global.__inMemoryMintStats)
                global.__inMemoryMintStats = new Map();
            const store = global.__inMemoryMintStats;
            ...
            const out2 = {};
            if (h.lastSeenTs)
                out2.lastSeenTs = Number(h.lastSeenTs);
            if (out2.lastSeenTs)
                out2.ageSeconds = Math.max(0, Math.floor(Date.now() / 1000) - Number(out2.lastSeenTs));
            if (h.volume_60s)
                out2.volume_60s = Number(h.volume_60s);
            if (h.volume_3600s)
                out2.volume_3600s = Number(h.volume_3600s);
            if (h.approxLiquidity)
                out2.approxLiquidity = Number(h.approxLiquidity);
            if (h.eventCount)
                out2.eventCount = Number(h.eventCount);
            return out2;
        }
        catch (e) {
            return null;
 ...
 */
async function updateMintStatsFromEvent(mint, amount = 0, slot, tsSec) {
    try {
        if (!mint)
            return false;
        const rc = await getRedisClient().catch(() => null);
        ...
        if (rc) {
            ...
            try {
                        ...
                        try {
                            ...
                            if (Array.isArray(cache) && cache.length) {
                                ...
                                const found = cache.find((c) => {
                                    try {
                                        return String(c.tokenAddress || c.address || c.mint || '').toLowerCase() === norm;
                                    }
                                    catch {
                                        return false;
                                    }
                                ...
                                if (found) {
                                    ...
                                    if (liq && !isNaN(liq) && Number(liq) > 0)
                                        pipeline.hSet(key, 'approxLiquidity', String(Number(liq)));
                                }
                            }
                        }
                        catch (e) { }
                ...
                try {
                    await rc.hSet(key, 'lastSeenTs', String(nowSec)).catch(() => { });
                    await rc.expire(key, ttl).catch(() => { });
                }
                catch { }
            }
            return true;
                    ...
                    if (!data.approxLiquidity) {
                        ...
                        if (Array.isArray(cache) && cache.length) {
                            ...
                            if (found) {
                                const liq = Number(found.liquidity ?? found.liquidityUsd ?? found.marketCap ?? found.liq ?? found.approxLiquidity ?? found.volume ?? 0) || 0;
                                if (liq && !isNaN(liq) && Number(liq) > 0)
                                    data.approxLiquidity = Number(liq);
                            }
                        }
                    }
                }
...
async function getAccountInfo(pubkey) {
    try {
        globalThis.__heliusAccountCache = globalThis.__heliusAccountCache || new enrichHelpers_1.TTLCache(Number(process.env.HELIUS_ACCOUNT_CACHE_MS || 30000));
        const cache = globalThis.__heliusAccountCache;
        ...
        try {
            cache.set(key, res, Number(process.env.HELIUS_ACCOUNT_CACHE_MS || 30000));
        }
        catch (e) { }
        return res;
    }
...
async function getTokenSupply(pubkey) {
    return heliusRpc('getTokenSupply', [pubkey], 4000, 1);
}
function metadataPdaForMint(mint) {
    try {
        ...
        return web3_js_1.PublicKey.findProgramAddress(seeds, METADATA_PROGRAM).then(([p]) => p.toBase58());
    }
    catch (e) {
...
async function handleNewMintEvent(mintOrObj, users, telegram) {
    ...
    try {
        ...
        if (!earliestBlockTime) {
            ...
            if (!r || r.__error) {
                if (r?.__error)
                    console.log(`Enrich ${mint} error: ${r.__error}`);
                return null;
            }
            ...
            try {
                ...
                for (let i = 0; i < arrOldest.length; i++) {
                    ...
                    try {
                        ...
                        const instrs = (p?.transaction?.message?.instructions) || (p?.meta?.innerInstructions && Array.isArray(p.meta.innerInstructions) ? p.meta.innerInstructions.flatMap((x) => x.instructions) : null) || [];
                ...
                if (!found) {
                    ...
                    for (const entry of arr) {
                        const sig = entry?.signature || entry?.txHash || entry?.tx_hash || null;
                        let btv = entry?.blockTime ?? entry?.block_time ?? entry?.blocktime ?? entry?.timestamp ?? null;
                        if (btv && btv > 1e12)
                            btv = Math.floor(Number(btv) / 1000);
                        if (btv) {
                    ...
                    if (!earliestBlockTime) {
                        const sigTry = arr[arr.length - 1]?.signature || arr[arr.length - 1]?.txHash || arr[arr.length - 1]?.tx_hash || null;
                        if (sigTry) {
                            try {
                                ...
                                if (pbt)
                                    earliestBlockTime = Number(pbt > 1e12 ? Math.floor(pbt / 1000) : pbt);
        ...
        if (metadataExists && metadataPda) {
            try {
                ...
                if (metaSigsRes && !metaSigsRes.__error) {
                    const metaArr = Array.isArray(metaSigsRes) ? metaSigsRes : (metaSigsRes.result ?? metaSigsRes);
                    if (Array.isArray(metaArr) && metaArr.length) {
                        ...
                        if (metaSig) {
                            try {
                                ...
                                const pbt = parsedMeta?.blockTime ?? parsedMeta?.result?.blockTime ?? parsedMeta?.result?.block_time ?? null;
                                if (pbt) {
                                    const pnum = Number(pbt > 1e12 ? Math.floor(pbt / 1000) : pbt);
                                    if (!earliestBlockTime || pnum < earliestBlockTime) {
                                        earliestBlockTime = pnum;
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    const list = Array.isArray(arr) ? arr : (arr.result ?? arr);
        ...
        try {
            ...
            if (sup && sup.value && (sup.value.amount !== undefined))
                supply = Number(sup.value.amount);
        }
        catch (e) { }
        ...
        try {
            ...
            if (prev === true) {
                ...
                if (validated)
                    console.log(`ValidatedNewMint (skipped-notify): ${mint} firstBlockTime=${earliestBlockTime} ageSeconds=${ageSeconds} metadata=${metadataExists} supply=${supply}`);
                return detection;
            }
        }
        catch (e) { }
        if (validated && users && telegram) {
            for (const uid of Object.keys(users)) {
                try {
                    ...
                    if (!u || !u.strategy || u.strategy.enabled === false)
                        continue;
                    const strat = (0, strategyNormalizer_1.normalizeStrategy)(u.strategy);
                    ...
                    const matches = await (0, strategy_1.filterTokensByStrategy)([tokenObj], strat);
                    if (Array.isArray(matches) && matches.length) {
                        ...
                        if (u.strategy.autoBuy !== false && Number(u.strategy.buyAmount) > 0) {
                            try {
                                const amount = Number(u.strategy.buyAmount);
                                const result = await (0, tradeSources_1.unifiedBuy)(mint, amount, u.secret);
                                if (result && (result.tx || result.success)) {
                                    try {
                                        await (0, strategy_2.registerBuyWithTarget)(u, { address: mint, price: tokenObj.price || 0 }, result, u.strategy.targetPercent || 10);
                                    }
                                    catch { }
                                    const { fee, slippage } = (0, tradeMeta_1.extractTradeMeta)(result, 'buy');
                                    u.history = u.history || [];
                                    ...
                                    try {
                                        await telegram.sendMessage(uid, `✅ AutoBuy executed for ${mint}\nTx: ${resTx}`);
                                    }
                                    catch { }
                                }
                            }
        ...
        if (validated)
            console.log(`ValidatedNewMint: ${mint} firstBlockTime=${earliestBlockTime} ageSeconds=${ageSeconds} metadata=${metadataExists} supply=${supply}`);
        else
            console.log(`CandidateMint (unvalidated): ${mint} firstBlockTime=${earliestBlockTime} ageSeconds=${ageSeconds} metadata=${metadataExists} supply=${supply}`);
        ...
        try {
            ...
            if (rc) {
                ...
                try {
                    await rc.setEx(key, ttl, '1').catch(() => { });
                }
                catch (e) { }
            }
        }
...
const _hh_handle_cache_ttl_ms = Number(process.env.HELIUS_HANDLE_CACHE_TTL_MS || (process.env.HELIUS_HANDLE_CACHE_TTL_S ? Number(process.env.HELIUS_HANDLE_CACHE_TTL_S) * 1000 : 60000));
let _hh_cache;
try {
    _hh_cache = new enrichHelpers_1.TTLCache(_hh_handle_cache_ttl_ms);
}
catch (e) {
    _hh_cache = new Map();
}
const _hh_inflight = new Map();
let _hh_active = 0;
const _hh_queue = [];
const _hh_max = Number(process.env.HELIUS_HANDLE_CONCURRENCY ?? 3);
function _hh_enqueue(fn) {
    _hh_queue.push(fn);
}
function _hh_dequeue() {
    if (_hh_queue.length === 0)
        return;
...
async function handleNewMintEventCached(mintOrObj, ttlSec) {
    try {
        const mint = typeof mintOrObj === 'string' ? mintOrObj : (mintOrObj?.mint || null);
        if (!mint)
            return null;
        ...
        let cached = null;
        try {
            cached = (typeof _hh_cache.get === 'function') ? _hh_cache.get(key) : _hh_cache.get(key);
        }
        catch (e) {
            cached = null;
        }
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (typeof _hh_cache.set === 'function') {
                                try {
                                    _hh_cache.set(key, entry, ttl * 1000);
                                }
                                catch (e) {
                                    try {
                                        _hh_cache.set(key, entry);
                                    }
                                    catch (ee) { }
                                }
                            }
                            else if (typeof _hh_cache.set === 'function') {
                                try {
                                    _hh_cache.set(key, entry);
                                }
                                catch (e) { }
...
async function runFastDiscoveryCli(opts) {
    ...
    for (const it of data) {
        ...
        if (!map.has(mint))
            ...
            map.set(mint, Math.max(map.get(mint), liq));
        if (map.size >= topN * 3)
            break;
    }
    ...
    for (let i = 0; i < top.length; i += concurrency)
        batches.push(top.slice(i, i + concurrency));
    ...
    async function collectSignaturesFull(address, maxCollect = 2000) {
        const out = [];
        let before = null;
        const limit = 1000;
        for (let i = 0; i < 5 && out.length < maxCollect; i++) {
            try {
                ...
                if (arr.length < limit)
                    break;
                before = arr[arr.length - 1].signature || arr[arr.length - 1].txHash || arr[arr.length - 1].tx_hash || null;
            }
            catch (e) {
                break;
            }
    ...
    for (const batch of batches) {
        await Promise.all(batch.map(async (t) => {
            try {
                if (!heliusUrl) {
                    results.push({ mint: t.mint, error: 'no-helius-url' });
                    return;
                }
                ...
                if (r && r.__error) {
                    ...
                    const alt = config_1.HELIUS_RPC_URL;
                    if (alt && alt !== heliusUrl) {
                        ...
                        if (r2 && r2.__error) {
                            ...
                            try {
                                const solscanUrl = `${config_1.SOLSCAN_API_URL}/token/${t.mint}/transactions`;
                                const headers = {};
                                const sk = (0, config_1.getSolscanApiKey)(true);
                                if (sk) {
                                    headers['x-api-key'] = sk;
                                    try {
                                        const { maskKey } = await Promise.resolve().then(() => __importStar(require('./config')));
                                        console.log(`[FastFetcher->Solscan] using key=${maskKey(sk)}`);
                                    }
                                    catch (e) { }
                                }
                                const sresWrap = await (0, enrichHelpers_1.withTimeout)(axios_1.default.get(solscanUrl, { timeout: timeoutMs, headers }), timeoutMs + 200);
                                if (!sresWrap.ok) {
                                    // mark as solscan failure
                                }
                                else {
                                    const sres = sresWrap.result;
                                    const arr = sres.data ?? [];
                                    const first = Array.isArray(arr) ? arr[0] : null;
                                    const bt = first?.blockTime || first?.time || first?.timestamp || null;
                                    if (bt) {
                                        const meta = (0, enrichHelpers_1.makeSourceMeta)('solscan', true, { latencyMs: Date.now() - nowSec * 1000, raw: first });
                                        results.push({ mint: t.mint, firstBlockTime: Number(bt), ageSeconds: nowSec - Number(bt), source: 'solscan', earliestSignature: (first && (first.signature || first.txHash || first.tx_hash)) || null, __meta: meta });
                                        hostState[hk] = state;
                                        return;
                                    }
                                }
                        ...
                        let sig2 = null;
                        try {
                            const withBt2 = arr2.filter((x) => x && (x.blockTime || x.block_time || x.blocktime));
                            if (withBt2.length) {
                                let min = withBt2[0];
                                for (const it of withBt2) {
                                    const b = it.blockTime ?? it.block_time ?? it.blocktime ?? 0;
                                    if (b && b < (min.blockTime ?? min.block_time ?? min.blocktime ?? Infinity))
                                        min = it;
                                }
                        ...
                        if (bt2 && bt2 > 1e12)
                            bt2 = Math.floor(Number(bt2) / 1000);
                        if (!bt2) {
                ...
                if (earliestSig) {
                    ...
                    if (sigTry) {
                        try {
                            ...
                            const pbt = parsed?.blockTime ?? parsed?.result?.blockTime ?? parsed?.result?.block_time ?? parsed?.result?.blocktime ?? null;
                            if (pbt)
                                minBt = Number(pbt > 1e12 ? Math.floor(pbt / 1000) : pbt);
                            earliestSig = sigTry;
                        }
                        catch (e) { }
                    }
                    ...
                    if (ageNow <= 600) {
                        ...
                        if (Array.isArray(fullSigs) && fullSigs.length) {
                            ...
                            for (const s of fullSigs) {
                                ...
                                if (btv && btv > 1e12)
                                    btv = Math.floor(Number(btv) / 1000);
                                if (btv && (!fullMin || Number(btv) < fullMin)) {
                                    fullMin = Number(btv);
                                    candidateSig = s?.signature || s?.txHash || s?.tx_hash || null;
                                }
                            }
                            if (candidateSig) {
                                try {
                                    ...
                                    if (pbt) {
                                        const pnum = Number(pbt > 1e12 ? Math.floor(pbt / 1000) : pbt);
                                        if (!fullMin || pnum < fullMin)
                                            fullMin = pnum;
                                    }
                                }
                                catch (e) { }
                            }
                            if (fullMin && (!minBt || fullMin < minBt))
                                minBt = fullMin;
                        }
                    }
                }
...
async function fetchLatest5FromAllSources(n = 5) {
    const axiosLocal = axios_1.default;
    const heliusEvents = [];
    try {
        const { getRecentHeliusEvents } = require('./heliusWsListener');
        const evs = getRecentHeliusEvents();
        for (const e of evs) {
            if (e && e.mint)
                heliusEvents.push(e.mint);
            if (heliusEvents.length >= n)
                break;
        }
    }
    catch (e) { }
    ...
    try {
        ...
        if (dsRaw) {
            if (dsRaw.data)
                data = dsRaw.data;
            else if (Array.isArray(dsRaw))
                data = dsRaw;
            else if (dsRaw.__meta && dsRaw.data)
                data = dsRaw.data;
        }
        for (const it of data) {
            const m = extractMintFromItemLocal(it);
            const norm = (0, tokenUtils_1.normalizeMintCandidate)(m);
            if (norm && !dex.includes(norm))
                dex.push(norm);
            if (dex.length >= n)
                break;
        }
    ...
    const heliusHistory = [];
    try {
        const parseUrlTemplate = config_1.HELIUS_PARSE_HISTORY_URL;
        if (parseUrlTemplate) {
    ...
    async function maybeValidate(list) {
        if (!shouldValidate)
            return list.slice(0, n);
        const out = [];
        const concurrency = Number(process.env.HELIUS_VALIDATE_CONCURRENCY || 3);
        const limiter = (0, enrichHelpers_1.createLimiter)(concurrency);
    ...
    const heliusEventsNorm = Array.from(new Set(heliusEvents)).map(x => (0, tokenUtils_1.normalizeMintCandidate)(x)).filter(Boolean);
    const heliusHistoryNorm = Array.from(new Set(heliusHistory)).map(x => (0, tokenUtils_1.normalizeMintCandidate)(x)).filter(Boolean);
    return {
...
async function testFetchEnrichFilterLatest() {
    try {
        const limit = 5;
        // 1) fetch latest candidates from combined sources
        const subs = await fetchLatest5FromAllSources(limit);
        const mints = Array.from(new Set([...(subs.heliusEvents || []), ...(subs.dexTop || []), ...(subs.heliusHistory || [])])).slice(0, limit);
        ...
        try {
            ...
            hf = ff && (ff.handleNewMintEvent || ff.default && ff.default.handleNewMintEvent);
        }
        catch (e) { }
        if (!hf) {
            // fallback: use enrichTokenTimestamps on single token objects
            const { enrichTokenTimestamps } = require('./utils/tokenUtils');
            hf = async (evt) => { const addr = evt && (evt.mint || evt.address); const token = { tokenAddress: addr }; await enrichTokenTimestamps([token], { batchSize: 1, delayMs: 0 }); return token; };
        }
        // 3) enqueue enrich jobs and wait for them
        const promises = mints.map((m) => mgr.enqueue({ mint: m, detectedAt: Date.now() }, hf));
        const results = await Promise.all(promises.map(p => p.catch((e) => ({ error: String(e && e.message || e) }))));
        ...
        const tu = require('./utils/tokenUtils');
        // map tokens by mint
        const candidates = mints.map((m) => {
            const found = (cache || []).find((c) => (c.tokenAddress || c.address || c.mint || '').toString() === (m || '').toString());
