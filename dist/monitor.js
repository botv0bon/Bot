...
async function toTsFromValue(v) {
    if (v == null)
        return null;
    if (typeof v === 'number') {
        if (v > 1e12)
    ...
    while (running) {
        try {
            ...
            for (const mint of candidates.slice(0, 200)) {
                const t = (allTokens || []).find((x) => String(x.tokenAddress || x.address || x.mint || '') === String(mint));
                if (!t)
                    continue;
                let ageMin = undefined;
                if (typeof t.ageMinutes === 'number')
                    ageMin = Math.floor(t.ageMinutes);
                if (typeof ageMin === 'undefined') {
                    ...
                    for (const k of cs) {
                        ...
                        if (ts) {
                            ageMin = Math.floor((now - ts) / 60000);
                            t._ageSource = k;
                            t._ageTs = ts;
                            break;
                        }
                ...
                if (typeof ageMin === 'undefined' && t.freshnessDetails) {
                    ...
                    if (ts2) {
                        ageMin = Math.floor((now - ts2) / 60000);
                        t._ageSource = 'freshnessDetails';
                        t._ageTs = ts2;
                    }
                }
            ...
            if (found.length)
                console.log(new Date().toISOString(), 'matches:', found.length, JSON.stringify(found.slice(0, 10)));
        }
        catch (e) {
