...
# Find tokens with age <= 30s and have a Jupiter route (liquidity)
...
echo "تشغيل البحث لمدة ${DURATION}s — LIMIT=${LIMIT} — AGE_THRESHOLD=${AGE_THRESHOLD}s"
  ...
  while IFS= read -r sig; do
    ...
    while IFS= read -r mint; do
      ...
      if [ "$age" -le "$AGE_THRESHOLD" ] && [ "$route" = "YES" ]; then
        ...
        seen_mints[$mint]=1
        echo -e "${mint}\t${age}\t${route}\t${pump}\tACCEPTED\t${quote_compact}\t${pump_compact}"
      fi
